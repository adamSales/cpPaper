...
print(timeStateYear <- ggplot(secByStud2,aes(Yr,time))+geom_boxplot(outlier.shape=NA)+
...
secStateYear <- ggplot(secByStud2,aes(Yr,numSec))+geom_boxplot(outlier.shape=NA)+
...
probStateYear <- ggplot(secByStud2,aes(Yr,nprob))+geom_boxplot(outlier.shape=NA)+
...
print(unitsStateYear <- ggplot(secByStud2,aes(Yr,numUnit))+geom_boxplot(outlier.shape=NA)+geom_jitter(data = function(x) dplyr::filter_(x, ~ outlier), width=0.2)+facet_grid(~state)+labs(x='',y='# Units Worked Per Student')+coord_cartesian(ylim=c(0,55)))
@
...
\section{Mastery Learning: Working Units in Order---Or Not}
