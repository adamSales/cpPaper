...
times.
They are listed in the order they appear in the standard Algebra I curriculum.
...
For instance, in Figure \ref{fig:trans1}, the thick line from
...
<<transition1,fig.height=3,fig.width=4.5,dependson='secOrder'>>=
...
propTotCP <- sum(trans1)/sum(secOrderCP$year==1)
#trans1itionPlot(trans1)
...
for(i in 1:nsec) if(uuu[i]%in%rownames(trans1)) text(1,nsec-i+1,uuu[i],adj=c(1,NA))
for(i in 1:nsec) if(uuu[i]%in%colnames(trans1)) text(2,nsec-i+1,uuu[i],adj=c(0,NA))

for(rr in rownames(trans1))
 for(cc in colnames(trans1)){
#     if(trans1[rr,cc]>20){
      arrows(1.1,nsec-which(uuu==rr)+1,1.9,nsec-which(uuu==cc)+1,lwd=5*trans1[rr,cc]/sum(trans1[rr,]),
...
<<trans1legend,fig.width=1.6,fig.height=3>>=
par(mar=c(.1,.1,.1,.1))
plot(1,1,xlim=c(0,2),ylim=c(0,11),cex=0,xaxt='n',yaxt='n',xlab='',ylab='')
text(1,10,expression(underline(bold(paste(Legend)))))
text(1,8,'Thickness: Percent')
text(.5,7,'25%')
text(.5,6,'50%')
text(.5,5,'75%')
arrows(c(1.5,1.5,1.5),c(7,6,5),c(2,2,2),lwd=5*c(.25,.5,.75))
text(1,4,'Darkness: Number')
text(.5,3,'10')
text(.5,2,'25')
text(.5,1,'50+')
arrows(c(1.5,1.5,1.5),c(3,2,1),c(2,2,2),col=rgb(0,0,0,alpha=c(1/5,.5,1)))
...
\caption{Reassignment transition plot for study year 1. The units on
  ...
  along with their top two receiving units. The units are listed
  following their order in the standard Algebra I curriculum. There is an arrow from a
...
\begin{figure}
<<transition2,fig.height=5,fig.width=6.5,dependson='secOrder'>>=
...
propTotCP <- sum(trans2)/sum(secOrderCP$year==1)
...
for(i in 1:nsec) if(uuu[i]%in%rownames(trans2)) text(1,nsec-i+1,uuu[i],adj=c(1,NA))
for(i in 1:nsec) if(uuu[i]%in%colnames(trans2)) text(2,nsec-i+1,uuu[i],adj=c(0,NA))

for(rr in rownames(trans2))
 for(cc in colnames(trans2)){
#     if(trans2[rr,cc]>20){
      arrows(1.1,nsec-which(uuu==rr)+1,1.9,nsec-which(uuu==cc)+1,lwd=5*trans2[rr,cc]/sum(trans2[rr,]),
...
\subsection{Does Reassignment Depend on Student Ability?}

<<xirtMods,include=FALSE,dependson=c('data','setup')>>=
## don't worry about nesting, etc--we aren't reporting standard errors anyway
cpDat <- data%>%filter(!is.na(status))%>%group_by(field_id,unit,section,state,year,classid2,schoolid2)%>%
    summarize(cp=max(status)=='changed placement')
...
stud <- rbind(h1[,c(174:194)],h2[,c(176:196)])
stud <- stud[stud$field_id%in%cpDat$field_id,]
stud <- stud[order(stud$field_id),]
stud$xirt <- rowMeans(stud[,grep('Exirt2_',names(stud))],na.rm=TRUE)

cpDat$xirt <- stud$xirt[match(cpDat$field_id,stud$field_id)]

cpModSimp <- glm(cp~splines::ns(xirt,5)*year,data=cpDat)

cpModSt <- list()
for(st in c('TX','KY','MI','LA','CT')){
    cpModSt[[st]] <- update(cpModSimp,data=subset(cpDat,state==st))
}
...
\begin{figure}
<<xirtCP,fig.height=7,fig.width=6.5,dependson=c('data','setup','xirtMods')>>=
#load('cpXirtMods.RData')


xirtDat <- cpDat%>%group_by(field_id,state,year,classid2,schoolid2,xirt)%>%
    summarize(nsec=n_distinct(section,unit),propCP=mean(cp,na.rm=TRUE))
...
ggplot(subset(xirtDat,state!='NJ'),aes(xirt,propCP,size=nsec))+geom_point()+geom_line(aes(xirt,fitted),col='red',size=1)+facet_grid(year~state)+coord_cartesian(ylim=c(0,0.5))
...
\begin{figure}
<<cp-xirtClassroom1,dependson='xirtCP',fig.height=7,fig.width=6.5>>=
bbb2 <- data%>%filter(status=='changed placement')%>%group_by(classid2,year,state)%>%summarize(ncp=n_distinct(field_id,section,unit),nstud=n_distinct(field_id),nsec=n_distinct(section))

cpcls1 <- xirtDat%>%
    filter(classid2%in%bbb2$classid2[bbb2$year==1 & bbb2$ncp>=20] & state!='Overall')%>%
    arrange(state)
cpcls1$classid2 <- factor(cpcls1$classid2,levels=unique(cpcls1$classid2))

cpcls1 <- cpcls1%>%group_by(classid2)%>%mutate(maxCP=max(propCP,na.rm=TRUE),propCP=propCP/maxCP)%>%ungroup()

ggplot(cpcls1,aes(xirt,propCP,size=nsec,color=state))+
    geom_point()+geom_smooth(size=1,method='lm')+facet_wrap('classid2')+scale_size_continuous(range=c(0.2,1))+
    ylim(0,1)
...
\begin{figure}
<<cp-xirtClassroom2,dependson='cp-xirtClassroom1',fig.height=7,fig.width=6.5>>=

cpcls2 <- xirtDat%>%
    filter(classid2%in%bbb2$classid2[bbb2$year==2 & bbb2$ncp>=20] & state!='Overall')%>%
    arrange(state)
cpcls2$classid2 <- factor(cpcls2$classid2,levels=unique(cpcls2$classid2))

cpcls2 <- cpcls2%>%group_by(classid2)%>%mutate(maxCP=max(propCP,na.rm=TRUE),propCP=propCP/maxCP)%>%ungroup()

ggplot(cpcls2,aes(xirt,propCP,size=nsec,color=state))+
    geom_point()+geom_smooth(size=1,method='lm')+facet_wrap('classid2')+scale_size_continuous(range=c(0.2,1))+
    ylim(0,1)
