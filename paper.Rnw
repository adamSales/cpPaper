...
\Sexpr{round(trans1['4-Quadrant Linear Graphs','4-Quadrant Linear Graphs']/
  sum(trans1['4-Quadrant Linear Graphs',])*100)}\%
of the reassignments begun in ``4-Quadrante Linear Graphs'' end in the
same unit.
Yet it is also fairly faint, since it only represents
\Sexpr{trans1['4-Quadrant Linear Graphs','4-Quadrant Linear Graphs']}
reassignments.
...
for(i in 1:nsec) if(uuu[i]%in%rownames(trans1)) text(1,nsec-i+1,uuu[i],adj=c(1,NA),cex=0.8)
for(i in 1:nsec) if(uuu[i]%in%colnames(trans1)) text(2,nsec-i+1,uuu[i],adj=c(0,NA),cex=0.8)
...
students to be reassigned to the next unit in the curriculum.
On the other hand, it is rare for students to be reassigned
...
for(i in 1:nsec) if(uuu[i]%in%rownames(trans2)) text(1,nsec-i+1,uuu[i],adj=c(1,NA),cex=0.8)
for(i in 1:nsec) if(uuu[i]%in%colnames(trans2)) text(2,nsec-i+1,uuu[i],adj=c(0,NA),cex=0.8)
...
They may help guide research


\subsection{Effects on Subsequent Mastery}
<<>>=
@

\subsection{Effects on Post-Test Scores}
