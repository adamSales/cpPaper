...
\begin{figure}
<<transition1,fig.height=3,fig.width=6.5,dependson='secOrder'>>=

trans <- with(filter(secOrderCP,year==1),table(unitName,nextUnitName))

## top senders and receivers
cols <- which(colSums(trans)>30)
rows <- which(rowSums(trans)>30)
...
for(r in rows)
    while(sum(trans[r,cols2])/sum(trans[r,])<0.75){
    nextBest <- which.max(trans[r,seq(ncol(trans))[-cols2]])
    cols2 <- c(cols2,which(colnames(trans)==names(nextBest)))
}

for(cc in cols){
 #   print('new col')
  #  print(cc)
    while(sum(trans[rows2,cc])/sum(trans[,cc])<0.75){
        nextBest <- which.max(trans[seq(nrow(trans))[-rows2],cc])
        rows2 <- c(rows2,which(rownames(trans)==names(nextBest)))
    }
...
trans <- trans[rows,cols]
## trans <- trans[rowSums(trans)>50,colSums(trans)>10]
## trans <- trans[rowSums(trans)>50,colSums(trans)>10]
## trans <- trans[rowSums(trans)>50,colSums(trans)>10]


propTotCP <- sum(trans)/sum(secOrderCP$year==1)
...
#     if(trans[rr,cc]>20){
      arrows(1.1,nsec-which(uuu==rr)+1,1.9,nsec-which(uuu==cc)+1,lwd=5*trans[rr,cc]/sum(trans[rr,]),
       col=rgb(0,0,0,alpha=ifelse(trans[rr,cc]>50,1,trans[rr,cc]/50)))
}
...
trans <- with(filter(secOrderCP,year==2),table(unitName,nextUnitName))
